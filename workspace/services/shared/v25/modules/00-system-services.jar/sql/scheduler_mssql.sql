...
select 
  t.*, 
  ISNULL( (select 1 from sys_task_suspended p where p.id=t.id), 0 ) AS suspended, 
  ISNULL( (select 1 from sys_task_error p where p.id=t.id), 0 ) AS error 
from sys_task t 
